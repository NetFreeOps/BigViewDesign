...
const ToolTips = ref(false)//拖放位置提示框
	...
	const changeZoom =  (zooms: number) => {
		...
		//参考线辅助函数
		const getRefLineParams =  (params: { vLine: any; hLine: any; }) => {
			const {
				vLine,
				hLine
			} = params
		...
		//将左侧选中组件添加到中间设计器中
		const AppendComponentToDesign = (res) => {
			// console.log(res)
			// 获取uuid
			let ids = uuid()
			//let ids = NewUUID()
		...
		//显示工具栏
		const onActivated = (res: { datas: any[]; id: any; }) =>{
